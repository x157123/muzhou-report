...
| **设计器** | FortuneSheet 提供的 Excel 内核（合并、样式、公式、多 Sheet）；左侧字段拖拽落格；右侧单元格属性面板（扩展方向 / 分组 / 聚合 / 格式化，并显示选中单元格的宽高 px / mm）；打印设置（纸张 / 方向 / 页边距 / 缩放 / 打印区域 / 页头页尾 / 水印）并在画布上实时显示分页线，可选「仅当前工作表」或「全部工作表」——多 Sheet 报表里宽表用 A3 横向、窄表用 A4 纵向互不影响；开启「限制宽度」后拖动列宽不会把要打印的列撑出纸张宽度 |
...
| **按数据拆单据** | 设一份模板，按主接口的数据列表**每行拆一份**，两种出法：**每条数据一个 sheet**（sheet 名取该行任一字段），或**每条数据一页**（N 份拼在同一张 sheet 里，每份开头打分页符 —— Excel 里是一张连续的表，打印仍是一条一页）。单据、工资条、成绩单这类报表设计一次即可 |
...
### 按数据拆单据（每条数据一个 sheet / 每条数据一页）

设一份模板，按数据列表拆成 N 份（一条数据一张单据）：

1. 把要拆的数据集设为**主接口**（左侧数据集面板点 ☆），它得是**集合型**。
2. 打印设置 → **输出** 页签 → 输出方式选一种：
   - **每条数据一个 sheet** —— 拆成 N 张工作表，再选 sheet 名取哪个字段
     （留空就是「第1条」「第2条」…，重名自动挂 `(2)`）。**模板本身有多张 sheet 时选不了这项**：
     M 张模板 × N 条数据 = M×N 张工作表，同一条数据的几张单据被打散在整个工作簿里对不上号，
     这种情况用下面那种。
   - **每条数据一页** —— N 份**首尾相接拼在同一张 sheet 里**，每份开头打一个分页符。
...
两种拆法的数据是一样的：模板整份复制 N 遍（N = 主接口的行数），第 i 份只喂第 i 行数据，
...
再把主表设为主接口 + 「每条数据一个 sheet」或「每条数据一页」，出来的就是一条主数据一张带明细的单据。
...
- **按数据拆单据时页码按单据重编**：`${page}` / `${pages}` 数的是**这一份单据**的第几页 / 共几页，
  ...
  页数，改不了；**Word** 每张工作表（= 一节）能重新数，但「每条数据一页」那种拼在同一张工作表里的
  拆法在 Word 里重不了编，页码仍是连续的。要单据页码分毫不差就导 PDF。
- **`${sheet}` 在「每条数据一页」时印的是本单据的名字**：这个模式把 N 份拼回同一张工作表，
  工作表名对每一份都一样，所以页头页尾里的 `${sheet}` 改从**打印设置 → 输出 → 单据名**
  那个字段取（配的是同一项，换成「每条数据一个 sheet」时它就是工作表名；留空 = 第1条、第2条…）。
...
- 一行比一页正文还高时（长备注格常见），**PDF 会把这一行横着劈开跨页印**：上一页印到页边距为止，
  下一页从断点接着印，所以表头下面不会凭空空掉一整页。切口只按高度算、不躲字，落在切口上的
  那一行文字上下两页各露半个。Excel / Word 的超高行由它们自己排（Word 的表格行默认允许跨页断行）。
