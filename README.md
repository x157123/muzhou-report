...
| **导出文件名** | 打印设置的「导出」页签里配：**报表名 + 主接口字段值 / 当前时间 / 主接口参数**拼成文件名（连接符可改，报表名也能关掉），一批单据导出来一眼能分清哪份是哪份。字段值**只在主接口恰好一条数据时才拼**（多条时第一条代表不了整份，那几段跳过），当前时间与参数不看数据、多条时照样在；取不到值的段整段跳过，文件名里不能用的字符自动洗掉，一段都拼不出来就退回报表名 |
...
2. 「拼接字段」多选，按选中顺序拼，三类可以混着来：
   - **主接口字段**（例：单号 + 仓库 → `销售出库单_SO-001_华东仓`）；
   - **当前时间**，`yyyyMMddHHmmss`（例：`销售出库单_20260816143005`）；
   - **主接口参数**，取这次查询实际用的参数值（下拉里没有的参数名可以直接手打 `${参数名}` 回车）。
3. 「连接符」默认下划线。下面的「示例」实时显示拼出来的样子。

Excel / PDF / Word 三种导出都用这个名字。

**字段值只在主接口恰好取到一条数据时才拼** —— 取回 200 条时第一条代表不了整份，
拼出来的 `销售出库单_SO-001` 里其实装着 200 张单，名字反而误导，所以那几段整段跳过。
「当前时间」和「主接口参数」不看数据，这种一批一批导出的报表就靠它们分清是哪次、哪个条件导的。

取不到值的段整段跳过（不会留下空的连接符），文件名里不能用的字符（`\ / : * ? " < > |`）
自动去掉，一段都拼不出来时退回报表名。
...
- **表格预览最多显示前 10 张工作表**：「每条数据一个 sheet」能出成百上千张，
  整份挂进网页里的表格控件会把浏览器卡住。**这只是预览的显示上限** ——
  导出的 Excel / PDF / Word 与预览页的 **PDF 视图**都是完整的，一张都不会少；
  截断时页面上会写明一共几张。要逐张核对就切到 PDF 视图或直接导出。
...
- 按 sheet 单独设的打印设置以 **sheet 下标** 为 key（与 `cellConfigs` 一致），
  增删工作表时设计器按工作表 id 把它们搬到新下标上（被删那张的设置一并丢弃，会提示一句）。
  **拖动标签重排出纸顺序跟着变**（导出的 Excel / PDF / Word 与设计器标签栏一致），
  打印设置与单元格绑定仍跟着各自那张走。
