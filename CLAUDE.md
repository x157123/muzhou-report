...
**工具栏「边框」菜单是打了补丁的**（`frontend/build/fortuneSheetPatch.js`，逐条理由写在文件头部）：
颜色/线型那套 UI 全在 @fortune-sheet/react 内部的 `CustomBorder` 里，既不导出也没有配置项，
桥接层碰不到，1.0.4 已是最新版。所以在**构建期替换它的源码**（不动 node_modules，`npm ci` 后照样生效）。
修的是三件事：二级菜单用 `e.target` 找节点导致「划上去多半不弹」、菜单一关内部状态就复位导致
不回显上次选的颜色/线型、以及由此带出的**真 bug** —— 重开菜单后再点一次颜色会把上次的线型
