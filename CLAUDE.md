...
（`ReportVersionResolver`，纯 POJO 可直接测）。**允许多版盖住同一刻**，重叠那一段归
**起点更晚**的那一版（起点相同则版本号大的赢）——「两个类型共用同一段时间」是正常配法，
所以重叠不拦（只拦单版自身 `to ≤ from` 这种空区间，`ReportVersionServiceImpl#checkRange`）。
早先只存起点、右端靠排序推：说不出「这一版 8/1 到期、之后回到常规版式」（推导出来的右端必定
被下一版接住，留不出空洞），两版也排不进同一条时间轴。**改法是等价扩展** —— 「重叠时起点更晚
的赢」与原先「取起点 ≤ 判定值的最后一个」是同一条规则，老数据 `effective_to` 全为 NULL，
行为一字不变，不必洗数据。一版都没盖住（落在空洞里）与判定值取不到一样用默认版本
（`is_default`，报表内恰好一条，不能停用/删除）；**停用的版本不参与**（临时回滚版式就靠这个）。
...
- **条件挂在版本行上，不是报表级** —— 报表级只放得下一套判定依据，放不下「每一版各自适用于什么」。
  老版本没有这一列 = 无条件匹配，所以这一维对老报表零影响。
- **特异度优先**：匹配到的版本里只留条件数最多的那一批（「类型=A 且 区域=华东」压过「类型=A」，
  都压过无条件那版），时间区间**只在这一批里判**。没有这一条的话，兜底版会跟配了条件的版本
  平起平坐、谁赢要看生效时间，配条件就白配了。**重叠也只在同一批里算**：前端
  `utils/version.js#versionIntervals` 同样按条件分组（`overlaps`/`beats` 与后端的
  `covers` + 排序一致），**两边必须一致**，否则界面上标出来的「重叠段归谁」是假的。
- **条件已把那批筛成唯一一版时，判定值取不到也照用它**；反过来，`field` 留空 = 时间这一维
  不参与（只按条件选），取那批里生效最晚的一版。「只有条件没有时间」是合法配法。
- **`gt/ge/lt/le` 只认数字，两边任一不是数字就跳过这一条**（视为满足、不参与 AND，
  `VersionMatchRuleDTO#compare`）：阈值分版说的是「金额大到某个程度换版式」，拿不到数字时它
  无从表态，判成不满足的话该字段为空的数据会整版落空掉到兜底版。**跳过的是判断不是这条条件**
  —— 特异度照算，否则同一份数据在「有数字」与「没数字」两种情况下落进不同批次，前端按条件
  指纹分组的重叠提示跟着对不上。
