...
**工具栏有一份构建期补丁**（`frontend/build/fortuneSheetPatch.js`，逐条理由写在文件头部）：
边框菜单、字号清单这些都写死在 @fortune-sheet/react 的 Toolbar 内部（组件不导出、也没有对应的
settings 项），桥接层碰不到，1.0.4 已是最新版。所以**构建期替换它的源码**
（不动 node_modules，`npm ci` 后照样生效）。眼下修两块：
...
（不是自己抄一份，抄一份的话升级后新增的按钮会全丢），剔完顺手合并连续的分隔符。
