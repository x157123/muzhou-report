...
- 环境依赖是**中文字体**：PDF 必须内嵌字体（`muzhou.report.pdf.font-path`，会自动探测系统字体）。
  这一款是**兜底**字体，单元格上设了字体名的走 `PdfFonts` 逐格解析（见下面「字体」一节）。
...
**续行（`rowOverflow=split`）在这条路上是「一个源行出好几行」**（`WordExporter#plan`）：Word 是
转结构不是渲染，切完得真的多出一行，所以只能**自己模拟一遍分页**才知道切在哪儿。三件事容易踩：
① **行高不能照搬 xlsx 那份** —— 它被 Excel 的 409.5pt 行高上限夹过，三页高的备注格在 xlsx 里
也就 409.5pt，照它判永远触发不了续行；得按 Word 自己重新分配过的列宽（`columnWidths`）
把文字折一遍再算（`measureRow`）。② 切完的段要 `w:cantSplit`，否则我们切一刀、Word 再切一刀，
两套断行叠加位置全乱；代价是估长了 Word 会把整段挤到下一页，所以正文高度按 `SPILL_SAFETY`
打了个折。③ **段内不要硬折行**：只把原文里真有的换行写成换行，段内交给 Word 自己再折一次
（它的字体与我们估的不一样，硬折出来长短不齐）——`RowSpill#wrapEstimate` 保证一个字符都不丢，
各段拼回去就是原文。另外 `applyMerges` 从此要按「源行 → 表格的哪几行」换算
（`partFirst`/`partLast`），横向合并要在**每一段**上都做一次，漏了就是第二段起整行多出几格。

### 字体：一个名字，四条路各认各的

单元格上只存一个**字体名**（FortuneSheet 样式里的 `ff`，`TemplateParser` 把它当普通样式原样带走，
引擎全程不知道有字体这回事），但四条出纸的路要的东西完全不同，这是这块所有坑的根源：

- **设计器画布**：浏览器渲染，要**本机**装了这款字体，或者有 `@font-face`；
- **xlsx / docx**：只写得进字体名（`ExcelExporter` 的 `font.setFontName`、`WordExporter` 的
  `run.setFontFamily`），**打开文件的那台机器**得有它；
- **PDF**：必须把**字体文件本身**内嵌进去，服务器上没有就印不出来。

于是有了**上传字体**（`mz_font` + 「字体管理」页 `/font`，CONTRACT §2/§3.6）：传一份上来，
`@font-face` 和 PDF 内嵌这两头就都有着落了。三件事必须一起记住：

- **字体名就是主键**（`ff` 里存的那个字符串），改名等于把老报表里绑着旧名字的格子打回默认字体，
  所以界面上写明了不建议改。**一个 id 的内容一经写入就不再变**（改名不动磁盘，换文件走
  「删了重传」）—— `PdfExporter` 那份「路径 → 已解析字体」的常驻缓存、各节点的本地缓存文件、
  下载接口上 30 天的 `Cache-Control`，三处都踩着这一条，哪天允许原地换文件了三处都得跟着失效，
  而其中两处根本没法跨节点通知。
- **字体文件的正本在库里**（`mz_font.file_data`），本地磁盘（`muzhou.report.font.dir`）只是缓存 ——
  **这一条是多节点部署逼出来的**：库是各节点唯一共享的东西，文件存在上传那个节点的磁盘上的话，
  A 节点传的字体在 B 节点导出时找不到、**静默退回默认字体**，「同一张报表导两次字体不一样」。
  各节点第一次用到某款字体时才落一份（`FontServiceImpl#ensureCached`，**先写临时文件再原子改名**
  —— 同节点两次导出可能同时发现本地没有，直接写目标文件的话后一个读到的是写了一半的内容），
  之后一直命中本地、不再读库。由此：`file_data` 上必须挂 `@TableField(select = false)`
  （一款中文字体十几 MB，列表页一页十行就是上百 MB），要字节走 `MzFontMapper#selectData`；
  **多节点必须切 MySQL**（默认那个内嵌 H2 是单节点的）。删除只删得掉本节点那份缓存，
  别的节点上是孤儿 —— 不必管，解析一律先查库，行没了就再也不会去碰它。
- **上传时真用 PDF 引擎加载一遍才收**：能不能印出来只有真要用它的那个引擎说了算，后缀是
  `.otf` 不代表 OpenPDF 读得了。这一刻不拦，就要等到某天有人导出那张报表才发现字体没生效，
  那时早没人记得是哪次上传的。ttf / otf **拿字节校验、不落盘**（`PdfFonts#probe`）——
  `BaseFont.createFont` 失败时文件句柄不一定收得回来（内存映射读的，JDK 9 之后 iText 那套
  `sun.misc.Cleaner` 反射清理早失效了），Windows 下随后 `Files.delete` 就撞「另一个程序正在
  使用此文件」，每传一次坏字体就在缓存目录里留一个孤儿。`.ttc` 没法从字节校验，只能先落盘。
- **「读不了」和「不许嵌入」是两回事，提示必须分开**：TTF 的 `OS/2` 表里那个 `fsType` 位是字体
  厂商声明的嵌入许可，取值 2 时 OpenPDF 拒绝内嵌，而 PDF 里的中文走 Identity-H **必须内嵌**，
  绕不过去。商业中文字体常这么设（同一款的不同版本还可能不一样：方正大标宋 `_GBK` 允许、
  老的「简体」版禁止）。`PdfFonts#embeddingRestricted` 认的是 OpenPDF 那句
  `cannot be embedded due to licensing restrictions` —— 对这种字体说「换一份 .ttf」毫无意义，
  文件本身好好的。另外**报错里别把缓存路径带给用户**，那是本节点的内部路径。
- **`.ttc` 字体集浏览器不收**（`@font-face` 用不了它），所以画布上退回默认字体、PDF 里正常，
  前端为此跳过它的 `@font-face` 并在管理页标了一句 —— 不标就是「传上去了但看不出效果」的报障。

PDF 那条路的解析在 `export/PdfFonts`，**三级往下找、先找到的赢**：上传字体 → 系统别名
（内置那 13 款在各系统字体目录下的位置，机器上有就该用上）→ 探测到的兜底字体。
两个必须守住的点：① **每个字体名一趟导出只问一次 `FontProvider#pathOf`**（那一问背后是查库 +
可能要落一次本地缓存），逐格问的话一张 500 行的报表就是几千次查库；② **`measure` 与
`drawText` 必须问同一个 `fonts.of(style)`**
—— 这里算出多高那边就画多高，两边不是同一款字体的话折出的行数对不上，
撑出来的行高与实际文字差一截，垂直居中的字上下两头会被切掉（同「自动换行的行高要自己算」
那一节里 `growWrapRows` 的道理）。粗体/斜体**仍然是描边与倾斜模拟的**：一个字体名对应的是
一个字体**文件**、不是一整个字族，手上没有它 Bold 的那一款。

前端那半在 `utils/fontList.js`：内置清单（**第一项是默认字体**，上传字体一律接在后面 ——
传一款字体不该把所有老报表的默认字体换掉）+ 启动时拉一次上传字体、注入 `@font-face`
并**等 `document.fonts` 就绪再挂载**（`main.js`）—— 不等的话首屏那次 `measureText`
量的是兜底字体的字宽，算出来的行高与随后画出来的不是一回事。
...
- `backend/.../export/ExcelExporter.java#applyWrapRowHeights`：导出时按**运行时真实文字**再算一遍
  —— 扩展出来的数据行文字长度模板期算不出来，只能在这里补。折行本身在
  `export/RowSpill#wrapEstimate` 一处（字宽按「全角 1 个字号、半角半个」估，不加载字体 ——
  服务器未必有那款字体），`WordExporter` 的续行用的是同一份：两边算出来的行数必须一样，
  否则「Excel 里 12 行、Word 切在第 11 行」。
...
**判「在不在本段外面」用的是 `Band#clipTop` 而不是 `Band#top`**：接着上一页往下印时 `top`
被抬到了正文区外面（抬多少 = 已经印掉的高度），拿它判等于「前几页印过的都算可见」，于是
那一整段又被写进这一页的内容流、还画在上页边距里。两者混用过一次，就是上面说的那个现象。

**切口要躲开文字行**（`RowSpill#snapCut` + `buildLineGrids` 记下的 `Geom#lineGrids`），否则一行字
上下两页各露半个。这里有三个必踩的坑：① 切口往上挪之后页底空出一条，**裁剪必须跟着挪到切口上**
...
两页各画半个，不然那半个字就彻底没了；③ **一行里每一格都要躲**，不只是撑得最高的那一格 ——
切口是整行一刀下去的，只躲开长备注那格，旁边那个短短的序号照样被从中间切成两半
（`lineGrids` 存的是一行**一组**网格，`snapCut` 取最靠上的那一刀）。网格只给
「比一页还高」的行算（别的行压根不会被切），而且**必须排在 `growWrapRows` 之后** ——
文字块摆在格子的哪个位置得等行高定下来。

**续行（`pageConfig.rowOverflow=split`）是横切的另一个出口，不是另一套排版**：分页照旧走
`splitRows`，区别只在 `drawCell` 把底色和边框**夹进本页**（`Band#clipTop`/`bottom`），于是切口
两边各是一个边框闭合的完整格子，看着就是「另起一行接着印」；外加超高行一律按顶对齐
（`verticalAlign(Geom,int,style)`），否则同一行里那个短序号按整行居中会落到中间那一页去，
跟自己那条数据差着两页纸。**对齐方式必须与算网格时问的是同一个方法**，两边不一致切口就对不上
文字行了。这一项**只作用于 PDF / Word**，xlsx 里表达不了（同水印，见 CONTRACT §4 能力表）。
...
**工具栏与右键菜单有一份构建期补丁**（`frontend/build/fortuneSheetPatch.js`，逐条理由写在文件头部）：
边框菜单、字号清单、右键菜单的渲染条件这些都写死在 @fortune-sheet/react 内部（组件不导出、
也没有对应的 settings 项），桥接层碰不到，1.0.4 已是最新版。所以**构建期替换它的源码**
（不动 node_modules，`npm ci` 后照样生效）。眼下修三块：
① **边框菜单**——二级菜单用 `e.target` 找节点导致「划上去多半不弹」、菜单一关内部状态就复位
导致不回显上次选的颜色/线型、以及由此带出的**真 bug**：重开菜单后再点一次颜色会把上次的线型
悄悄写回默认值（选颜色与选线型都会把**另一维**一并回写 Toolbar 的常驻状态）；
② **右键菜单的「行高」「列宽」对选中的格子也生效**——这两项官方本来就有（输入框、批量写回
选区、校验都是现成的），但只在**整行/整列**选区上渲染，框选一片格子右键时两项都不出现。
改成「整列选区不给行高、整行选区不给列宽」（同 Excel），其余情况两项都给。
**输入框的默认值改成选区的平均行高/列宽**（`mzAvgLen`，按 `config.rowlen`/`columnlen` 算、
四舍五入取整）：原文是「高矮不齐就显示空」，而框选一片格子时不齐才是常态，那个框十有八九是空的。
读了 config 就得把它补进 `getMenuElement` 那个 useCallback 的 deps，否则拖完行高不换选区直接右键
算的是老值。顺带修掉列宽输入框那个抄行高的 `max: 545`（它自己的校验放到 2038）；
③ **字号清单**补上 6/7/8（原本最小 9，报表的备注落款要用小字）。
两处必须一起挂：`build` 走 rollup 的 transform，`dev` 走 esbuild 预构建（**预构建不走 Vite 插件**）。
**改了补丁内容记得把 esbuild 插件 name 里的版本号加一** —— 预构建缓存的 hash 只认插件名，
不改名的话 dev 会接着用旧缓存。锚点找不到直接抛错中断构建，升级依赖时立刻暴露。

工具栏显示哪些按钮由 `ReportDesigner.vue` 的 `HIDDEN_TOOLBAR_ITEMS` 从官方默认清单里**剔除**得到
（不是自己抄一份，抄一份的话升级后新增的按钮会全丢），剔完顺手合并连续的分隔符（`withoutItems`）。
**右键菜单同一套路数**：`HIDDEN_CONTEXT_MENU_ITEMS` 剔掉排序选区/升降序/筛选选区（改的是模板本身，
出纸时的行序由数据集与扩展决定）与插入图片/插入链接（图片走单元格类型 `img`/`base64`，
超链接三条导出路都表达不了），`cellContextMenu` 与 `headerContextMenu` 两份都要剔
—— 排序那三项在行号/列标的右键里也有。
**数字格式那一整组是有意剔掉的**：格式统一由右侧「格式化类型」（`cellConfig.format`）控制，
工具栏那几个按钮写的是格子的 `ct.fa`，而渲染时 `ct` 会被 `CellFormatter` 按 `cellConfig` 重建，
模板里那份活不到出纸那一步 —— 留着就是「设计器里 1,234.00、导出全是 1234」。
...
**拆不拆是报表级的，谁跟着拆是模板级的**（`content.sheetSplits`，key = 模板下标，
`once` = 不跟着拆 / 缺省 = 跟着拆；界面在打印设置弹窗的「工作表」页签，一行一张逐个勾）：「第一张模板是清单列表、第二张是每条数据的详情」
就是这么配的 —— 没有这一维的话 `perRow` 会把清单页也复制 N 遍、每份还只喂一行数据。
模板按它切成**连续的几段**（`ReportRenderEngine#segments`），`once` 段整段只渲染一次、
主接口给它**全量**（就是探测时那份 rows，**不再打一次接口** —— 再打一遍可能拿到不一样的数据，
清单与单据就对不上号），`perRow` 段整段对每一行渲染一遍（段内仍是行优先）。
输出顺序照模板下标走：「清单、(数据1 明细/附页)、(数据2 明细/附页)…」。四件跟着来的事：
① **`perRowPage` 拼接时段与段之间必定断开**，判据是 `ConcatKey`（段号 + 打印设置，`SheetConcat`
只对它做 equals）—— 只比打印设置的话，两段设置碰巧相同时清单会被摞进单据那张里，
而清单本该是能在 Excel 里单独筛选排序的一张表；② `once` 那张**自成一份单据**
（`docIndexes` 从此是全局递增的单据号、不再是行下标），页码印「本清单第 x 页 共 y 页」；
③ **整份都标 `once` 时 `splitByRow()` 直接为 false**，退回普通渲染那条路，少一次主接口探测；
④ 逐行选版本时 **`once` 段恒取基准版本**（它对应「这一批数据」而不是某一行），
跟着拆的那几段按「第几个 `perRow` 段」与各版配对，段数不够就跳过那一条并记一次 warn。
...
**选版本是「时间先圈候选、条件后定唯一」两步**（`ReportVersionResolver#decide`，纯 POJO 可直接测）：
先留下生效区间盖住判定值的那些版本，按**时间优先级**排序（起点更晚的先，同起点版本号大的先，
起点为 NULL 的最后），再照这个次序逐个试匹配条件，**第一个条件全满足的就是它**。

**生效区间两端都存，左闭右开，`[effective_from, effective_to)`，两端都可为 NULL = 不限**。
**允许多版盖住同一刻**，重叠那一段**先轮到起点更晚**的那一版——「两个类型共用同一段时间」是
正常配法，所以重叠不拦（只拦单版自身 `to ≤ from` 这种空区间，`ReportVersionServiceImpl#checkRange`）。
早先只存起点、右端靠排序推：说不出「这一版 8/1 到期、之后回到常规版式」（推导出来的右端必定
被下一版接住，留不出空洞），两版也排不进同一条时间轴。**没配条件时「先轮到」就等于「赢」**，
与原先「取起点 ≤ 判定值的最后一个」是同一条规则，老数据 `effective_to` 全为 NULL 且没有条件，
行为一字不变，不必洗数据。一版都没圈进来（落在空洞里）、圈进来了但条件都不满足、
判定值取不到，三种都用默认版本（`is_default`，报表内恰好一条，不能停用/删除）；
**停用的版本不参与**（临时回滚版式就靠这个）。
**只有一个启用版本时直接短路**，不去探测取数 —— 绝大多数报表只有一版，不该为版本功能多打一次 SQL。

**时间之外的那几维是「匹配条件」**（`mz_report_version.match_rules`，`VersionMatchRuleDTO` 的
JSON 数组，同一版内多条是 AND；值取自主接口字段或报表参数，运算符 `eq/ne/in/notIn/contains/
empty/notEmpty` + 四个数字大小比较 `gt/ge/lt/le`）：时间说得清「什么时候起换版式」，
说不清「类型 A 走这版、类型 B 走那版」。五件事必须一起记住：

- **条件挂在版本行上，不是报表级** —— 报表级只放得下一套判定依据，放不下「每一版各自适用于什么」。
  老版本没有这一列 = 无条件匹配（任何数据都满足，于是它是同一时间段里的兜底），所以这一维
  对老报表零影响。
- **没有「特异度优先」这回事**：条件更多的版本**不会**天然压过更宽泛的，谁先被试到只看
  上面那条时间优先级。同一时间段里想让带条件的版本先于兜底版被试到，给它一个**更大的版本号**
  （界面上后建的版本天然如此）。曾经这里按条件数分批、条件多的整批压过条件少的，
  于是「时间」实际只在同特异度的版本之间起作用 —— 与「先按时间、再按条件」正相反。
- **时间排在条件前面**：判定字段配了却取不到值就直接 fallback，**不再**有「条件已唯一命中就
  照用」那条救济；时间窗口没盖住的版本压根不进候选，哪怕它的条件对得上。反过来 `field` 留空
  = 时间这一维不参与（只按条件选），照时间优先级取第一个条件满足的。「只有条件没有时间」
  是合法配法；两维都没配则直接用默认版本。
- **`gt/ge/lt/le` 只认数字，两边任一不是数字就跳过这一条**（视为满足、不参与 AND，
  `VersionMatchRuleDTO#compare`）：阈值分版说的是「金额大到某个程度换版式」，拿不到数字时它
  无从表态，判成不满足的话该字段为空的数据会整版落空掉到兜底版。
- **前端的重叠提示要跟着这条规则**：`utils/version.js#versionIntervals` 只在「盖住它的那一版是
  无条件的」时才报「这一段轮不到你」—— 盖住它的那版若自己也带条件，条件不满足时仍会轮到被盖的
  那一版，报重叠是误导。`overlaps`/`beats` 与后端 `covers` + `BY_TIME_PRECEDENCE` 必须一致。
- **探不探主接口跟着两维走**（`needsRow`）：判定值取自字段、**或哪一版的条件取自字段**才探；
  条件全取自参数时一次都不探。同理 `RenderServiceImpl#perRowVersioning` 判的是「判定跟着行走」
  而不再是「source=field」—— 漏了这一改，只配了条件的报表在 `perRowPage` 下会整份定一版。

**解析 JSON 在 `ReportVersionServiceImpl`（`parseRules`/`normalizeRules`），不在 resolver 里**
—— 那边是纯 POJO，不该认识 Jackson。**读宽容写严格**：读解析不了当无条件 + warn（一版的条件
写坏了不该让整张报表打不开），写的时候丢掉字段名空的条目、格式非法直接报错。
`match_rules` 跟着复制版本、复制报表、导入导出包一起走，漏一处就是「副本的条件没跟来」。
...
用对应版式」「一批单据里类型 A 和类型 B 各出各的版式」—— 时间与条件两维都是逐行判的，
`ReportVersionResolver#resolveByRow` 收的是**那一行**）：照旧是**换函数** —— 引擎的 `render`
多收一个 `versionPicker`，
