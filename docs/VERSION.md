...
> 7. **后来加了「匹配条件」这一维**（本文只有时间一维）：单据类型、区域这类离散维度没法用时间轴
>    表达，所以每一版再挂一组条件（`mz_report_version.match_rules`，同一版内多条 AND），
>    **条件先筛、时间后推，多版同时匹配时条件更具体的赢**。时间那一维（本文全部内容）原样不动，
>    没配条件的版本 = 无条件匹配，老报表零影响。落地细节见 CONTRACT §4.1。
> 8. **后来又把「只存起点」改成「两端都存」**（§1.3 与 §2 第 6 步作废）：加了
>    `effective_to`，区间 `[effective_from, effective_to)` 两端都由用户填、都可为空 = 不限。
>    §1.3 说「只存起点在结构上就不可能有洞」，可**有些洞是用户要的** —— 「这一版 8/1 到期、
>    之后回到常规版式」用推导出来的右端说不出口（右端必定被下一版接住），而「两个类型共用
>    同一段时间」更是压根排不进一条时间轴。于是**重叠改为允许**，重叠那一段归**起点更晚**的
>    那一版（起点相同则版本号大的赢）。这条规则与原先「取起点 ≤ 判定值的最后一个」等价，
>    所以**老数据（`effective_to` 全为 NULL）行为一字不变**，不必洗数据。
>    §3 那张「只读推导区间表」相应变成可编辑的两个日期框 + 重叠提示（配得出重叠就得看得见），
>    §5 版本管理弹窗里那一列同理。落地细节见 CONTRACT §2 / §4.1。
> 9. **匹配条件后来又添了四个数字大小比较**（`gt` / `ge` / `lt` / `le`）：金额、数量这类
>    **阈值分版**（「10 万以上走大额版式」）用离散的 `eq`/`in` 表达不出来。它们**只认数字**，
>    两边任一不是数字时**跳过这一条**（视为满足、不参与那一版的 AND）—— 阈值条件说的是
>    「数字大到某个程度时换版式」，拿不到数字时它无从表态，判成不满足的话该字段为空的数据
>    会整版落空掉到兜底版。**跳过的是判断、不是这条条件的存在**：特异度照算。落地见 CONTRACT §4.1。
