...
> 7. **默认字体不是「探测到的中文字体」，而是逻辑字体 `SANS_SERIF`**（§3.4 原本只写了探测那一档）。
>    第一版出图后看纸面才发现：中文字体几乎都是**等宽**的 —— 黑体的逗号、句点、百分号字宽都是
>    0.5em，而墨迹只有 0.09em，于是「1,944,998.13」排出来两边全是空档，看着像
>    「1, 944, 998. 13」。图表上到处是刻度和数值标签，这个毛病比正文里显眼得多。
>    所以顺序改成「显式配置 → 逻辑字体（先验它显示得出中文）→ 探测清单 → 兜底」：
>    西文按正常比例排，中文由 JDK 的字体配置回退给系统字体。
>    **这条只有真出一张图看过才发现得了**，纯单测锁不住（`ChartFontsTest` 只能锁住
>    「逗号明显窄于数字」这个可度量的代理指标）。
...
配置沿用 PDF 那一项（`muzhou.report.pdf.font-path`），**但默认那一档是逻辑字体不是探测到的字体文件**
（落地时改的，见开头偏差第 7 条）：

```
显式配置的字体文件 → 逻辑字体 SANS_SERIF（先验它显示得出中文）→ 探测清单 → 兜底
```

四件事要写清楚：

- **中文字体是等宽的**，拿它排数字会一格一格的 —— 所以没配字体时优先用逻辑字体，
  中文靠 JDK 的字体配置回退（`ChartFonts#canDisplayChinese` 验一遍再用）；
- **`.ttc` 用不了**（`createFont` 不收字体集），退回下一档并记一条 warn ——
  与前端 `@font-face` 跳过 ttc 是同一类问题，提示语要说明「PDF 里正常、图表里不是这款字」；
...
3. **图表里的字整张一款**（一期），单元格上设的字体名不生效；默认用系统字体（数字按西文比例、中文由系统回退），要指定某一款就配 `muzhou.report.pdf.font-path`（`.ttc` 读不了）。
