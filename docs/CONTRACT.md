...
```jsonc
{
  ...
  // 输出方式：single 单 sheet 输出（默认）/ perRow 主接口每条数据一个 sheet /
  // perRowPage 每条数据一页、拼在同一张 sheet 里。
  // 后两者的**拆分方式完全相同**（同一段代码）：整份模板复制 N 遍（N = 主接口行数），
  // 第 i 份只喂主接口的第 i 行，其它数据集每份都拿全量 —— 「一条数据一张单据」就是这么出的。
  // 区别只在出口：perRow 直接输出 N×M 张 sheet；perRowPage 再把它们**首尾相接拼成一张**
  ...
  // perRow 还多一条限制：**模板本身是多 sheet（sheets.length>1）时不许用**，设计器里那个选项
  // 直接禁掉、存下来的老值退回 single（`stores/designer.js#setSheetSplit` / `setSheets`）。
  // M×N 张 sheet 会把同一条数据的几张单据打散在整个工作簿里，翻起来对不上号；
  // 多 sheet 模板要「一条数据一份」用 perRowPage。引擎不拦这条（老报表照旧能渲染），
  // 它是设计期的约束。
  "splitMode": "single",
  // 单据名取主接口这一行的哪个字段；取不到值退回「第 n 条」。**perRow 与 perRowPage 共用这一项**：
  // perRow 拿它当 sheet 名（重名自动挂 (2)(3)；模板有多张 sheet 时补上模板名 `SO-001-明细`
  // —— 新报表选不到这种组合了，引擎这份逻辑留给老数据）；
  // perRowPage 拼成一张之后 sheet 名说不了话（整张只有一个名字，还是模板名），
  // 它改为落在 mzDocNames 上，供页头页尾里的 ${sheet} 逐份取用（见下）。
