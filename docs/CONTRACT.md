...
**允许重叠**，重叠那一段**先轮到 `effective_from` 更晚**的那一版（起点相同则 `version_no` 大的
先）—— 没配条件时「先轮到」就等于「赢」，与早先那套「取起点 ≤ 判定值的最后一个」等价，
所以**老数据（`effective_to` 全为 NULL、也没配条件）行为一字不变**：
...
**配了匹配条件时「先轮到」不等于「赢」**：重叠段里排在前面的那一版条件不满足就轮到下一版
（见 §4.1 的第 5、6 步）。所以**只有当盖住它的那一版是「无条件」的，被盖的那一版才真的
永远轮不到** —— 前端 `utils/version.js#versionIntervals` 的重叠提示按这条判，两边必须一致。
...
> ——报表名 + 主接口若干字段值 + `${now}` / `${参数}`（见 §4）。拼它要用主接口那一行数据，
> 只有渲染那一层拿得到，所以**前端一律从响应头读**（`utils/sheet.js#fileNameFromResponse`），
> 别在前端自己拼一份。
...
- 字体名要同时当 CSS 的 `font-family`、xlsx 的字体名和单元格的 `ff` 用，所以 `"' , < > \ / ; { }`
  一律不收（前后端各拦一道），长度 ≤ 32。
- **`/upload` 一次只收一款，批量上传是前端把它串行调多遍**（字体管理页的上传弹窗，一个文件一行）
  —— 没有 `/upload/batch` 是有意的：每款都要真用 PDF 引擎加载一遍才收（上面那两条），
  一批里混进一款不许嵌入的商业字体时，该报的是「这一款不行、其余照传」而不是整批回滚，
  而多文件一次提交要么退化成整批失败、要么得自己定义一套逐条结果的出参。并发也不合适：
  一款十几 MB，十款一起发能把带宽和服务端内存一起打满。
...
```jsonc
{
  ...
  // / perRow 每条数据一个 sheet（**界面上叫「每条数据一个 sheet」**）。
  // **拆分方式两者完全相同**：整份模板复制 N 遍（N = 主接口行数），第 i 份只喂主接口的第 i 行，
  // 其它数据集每份都拿全量 —— 「一条数据一张单据」就是这么出的。最多 200 条。
  // 区别**只在出口**：
  //   perRow    不拼接，直接输出 M×N 张 sheet（M = 模板张数），导出的 Excel 里就是那么多标签页。
  //             顺序同为行优先（数据1的模板1/2/3、数据2的模板1/2/3），单据名当 sheet 名。
  //   perRowPage 把那一摞**首尾相接拼成一张**（engine/SheetConcat），并在每份的起始行打行分页符
  //             （见 mzRowBreaks），于是 Excel 里是一张连续的表、打印出来仍是一条数据一页。
  // **「多 sheet 输出」不是「一条数据一张工作表」** —— 那个名字说的是「打印设置不同的相邻份会
  // 断成几张 sheet」，全部模板设置相同时结果就只有一张。要一条数据一个标签页得选 perRow。
  ...
  // perRow 共用这一项，那边直接拿它当 sheet 名（重名自动挂 (2)(3)；模板有多张 sheet 时
  // 补上模板名 `SO-001-明细`）。
  "sheetNameField": "order_no",
  // **哪几张模板跟着拆**，key = 模板下标（字符串，与 pageConfigs 同一套寻址）。
  // once = 不跟着拆（整份只渲染一次，主接口给它**全量**数据）/ perRow = 跟着拆（**缺省**）。
  // splitMode 是报表级的总开关（拆不拆、拆完拼不拼），这一项按模板决定谁参与 ——
  // 「第一张是清单列表、第二张是每条数据的详情」就靠它，没有它 perRow 会把清单页也复制 N 遍、
  // 每份还只喂一行数据。**只存改过的那些**，老报表一项都没有 = 全部跟着拆，行为一字不变。
  // 模板按这一项切成**连续的几段**（相邻同类合并），按模板下标顺序输出：
  // 不拆的段各出一份，拆的段整段对每一行渲染一遍（段内仍是行优先）。
  // 「清单 + 明细 + 附页」（清单标 once）出来就是「清单、(数据1 明细/附页)、(数据2 明细/附页)…」。
  // 三件跟着来的事：① perRowPage 下**段与段之间必定断开**，哪怕两段打印设置一模一样
  //    —— 清单摞进单据那张里，Excel 端就没法单独筛选排序了；
  // ② 不拆的那张**自成一份单据**（mzDocBreaks），页码印「本清单第 x 页 共 y 页」，
  //    不跟后面的单据连着数；${sheet} 取它的模板名；
  // ③ 整份都标 once 时拆分是恒等变换，直接退回普通渲染（少一次主接口探测）。
  // 逐行选版本时**不拆的那几段恒取基准版本**（它不属于任何一行），跟着拆的那几段按
  // 「第几个 perRow 段」与各版配对；那一版段数不够就跳过这一条并记 warn。
  "sheetSplits": { "0": "once" },
  ...
  // 下载下来的 Excel / PDF / Word 叫什么名字 = 报表名（可关）+ fields 里的若干段，用 separator 拼。
  // 每一段有三种写法，可以混着来：
  //   `orderNo`      主接口字段 —— **只在主接口恰好取到一条数据时才拼**；取回多条时那几段
  //                  整段跳过（第一条代表不了整份：拼出来的「销售出库单_SO-001」里其实装着
  //                  200 张单，名字反而误导）。0 条、取数报错同样跳过。
  //   `${now}`       当前时间，恒为 yyyyMMddHHmmss（紧凑时间戳，本身不含要洗掉的字符）
  //   `${paramName}` 参数值，取这次渲染实际用的那份参数（报表参数/全局参数/地址栏透传，见 §5）；
  //                  `now` 是保留名，同名参数让位。
  // 后两种**不看数据**，所以多条数据的那一批也仍分得清是哪次、哪个条件导出的 —— 它们存在的理由。
  // 取不到值的段整段跳过、不留空的连接符；`\ / : * ? " < > |` 与控制字符会被洗掉；
  ...
  // 「恰好一条才算数」那一条是它的 nameRow(rows)，RenderServiceImpl 照着它挑那一行；
  // 前端 utils/sheet.js#exportFileName 是设计器里那个「示例」用的同一套拼法，改一处要改另一处。
  "exportConfig": {
    "withReportName": true,             // 文件名以报表名开头，默认 true
    "fields": ["orderNo", "${now}"],    // 拼进文件名的那几段，**按数组顺序**拼；空 = 只用报表名
...
—— 这条只对 `perRow` 成立，`perRowPage` 拼过之后算不出来；逐行选版本时各版模板张数可以不同，
取模同样不成立，那种结果一律靠 `sheetPageConfigs` / `mzTemplateIndex`。
...
**先按生效时间段**（`[effective_from, effective_to)`，见 §2）圈出这一刻生效的那几版，
**再用每一版自带的匹配条件**（离散维度：单据类型、区域…）从中定位唯一一份。
时间那一维的判定值从哪来由这条**报表级**规则说了算：
...
**四个大小比较只认数字**（金额、数量这类阈值分版：`{"field":"amount","op":"ge","value":"100000"}`），
两边任一不是数字（空值、文本、日期串）时**跳过这一条**——视为满足、不参与那一版的 AND，
而不是判成不满足：阈值条件说的是「数字大到某个程度时换版式」，拿不到数字时它无从表态，
该由同一版里别的条件与生效时间去定案；判成不满足的话，该字段为空的那条数据会整版落空掉到兜底版。
...
3. 取时间判定值：`field` 主接口**第一行**的该字段（拆分报表是**这一条数据**的该字段）/
   `param` 报表参数 / `now` 渲染当日；归一化成 `LocalDateTime`：`java.sql.Date/Timestamp`、
   `LocalDate(Time)`、`java.util.Date`、epoch 毫秒（数字或纯数字串）、字符串
   `yyyy-MM-dd[ HH:mm[:ss]]`（也认 ISO 的 `T` 与毫秒尾巴）。解析不了当作取不到；
4. **配了判定字段却取不到值** → `fallback`：`default` 用默认版本 / `error` 抛 `BizException`
   （消息里带字段名）。**时间排在条件前面，判定值都拿不到就没有资格再往下比条件**；
   判定字段留空（只按条件选）而又一版条件都没配 → 两维都没得判，用默认版本；
5. **时间圈候选**：留下判定值落在 `[effective_from, effective_to)` 内的版本（两端为 NULL =
   那一端不限；没有判定值时全员是候选），并按**时间优先级**排序 —— **`effective_from` 更晚的
   排前面**，起点相同则 **`version_no` 大的**排前面（起点为 NULL 的排最后，它是「全时段那一版」）。
   这与「重叠时起点更晚的赢」是同一条规则，只是从「挑一个」变成「排个序」；
6. **条件定唯一**：照这个次序逐个试匹配条件，一个版本的条件要**全部**满足才算匹配
   （没配条件 = 无条件匹配，天然满足，于是它是同一时间段里的兜底）——
   **第一个满足的版本就是它**，后面的不再看；
7. 扫完一个都没命中 → `fallback`（`default` 用默认版本 / `error` 抛 `BizException`，
   消息里区分「时间就没圈出人」与「圈出来了但条件都不满足」）。

**没有「特异度优先」这回事**：条件更多的版本**不会**天然压过更宽泛的，谁先被试到只看第 5 步那条
时间优先级。同一时间段里想让带条件的版本先于兜底版被试到，给它一个**更大的 `version_no`**
（界面上后建的版本天然如此）。

**`field` 留空（只按条件选）时时间这一维直接不参与**，照时间优先级扫一遍取第一个条件满足的。
只有条件、没有时间是完全合法的配法。

**老数据（`effective_to` 全为 NULL、也没配条件）行为一字不变**：没有条件时第 6 步恒在第一个
候选上命中，等价于早先那条「取起点 ≤ 判定值的最后一个」。
...
**不是每张模板都跟着拆**：模板按 `content.sheetSplits` 切成连续的几段（`ReportRenderEngine#segments`），
标了 `once` 的段整段只渲染一次、主接口给它**全量**（清单列表），其余的段整段按行拆（单据详情）。
「第一张是清单、第二张是每条数据的详情」就是这么出的，输出顺序照模板下标走：
清单一份、然后一条数据一组详情。清单那份**自成一份单据**（页码不跟单据连着数），
`perRowPage` 拼接时**段与段之间必定断开**（`ConcatKey` = 段号 + 打印设置，`SheetConcat` 只对它做
equals）—— 打印设置碰巧相同时清单会被摞进单据那张里，而清单本该是能单独筛选排序的一张表。
清单段的主接口数据直接用探测时那一份 rows，**不再打一次接口**（再打一遍可能拿到不一样的数据，
清单与单据就对不上号了）。整份都标 `once` 时 `splitByRow()` 为 false，直接退回普通渲染。

拆分时还可以**逐行换版式**（见 §4.1「版本」）：`render` 多收一个
`Function<row, ReportContentDTO> versionPicker`，每行用它换模板与打印设置，取数那套一个字没改。
（**`once` 段恒取基准版本** —— 它对应的是「这一批数据」而不是某一行；跟着拆的那几段按
「第几个 `perRow` 段」与各版配对，那一版段数不够就跳过这一条并记一次 warn。）
