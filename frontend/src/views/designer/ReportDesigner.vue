...
/**
 * 工具栏里屏蔽掉的按钮：货币格式、百分比格式、减少小数、增加小数、数字格式、自动求和（Σ）、
 * 冻结、条件格式、筛选、超链接、图片、批注、数据验证、分列、定位条件、截图、搜索。
 * 剔完只剩「排版」这一类：撤销重做、格式刷、字体字号字形、颜色边框合并、对齐换行旋转。
 *
 * **数字格式那一整组是有意剔掉的**：格式统一由右侧单元格属性面板的「格式化类型」控制
 * （落在 `cellConfig.format` 上）。工具栏这几个按钮写的是格子自己的 `ct.fa`，而渲染时每个格子的
 * `ct` 都会被后端 `CellFormatter` 按 `cellConfig` 重建一遍（没配过格式的拿到的是 `General`），
 * 模板里那份 `ct.fa` 根本活不到出纸那一步 —— 留着就是「设计器里看着是 1,234.00、
 * 预览导出全是 1234」，比没有更误导。
 *
